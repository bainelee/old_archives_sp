...
const ICON_IDS: Array[String] = [
	...
	"confuse", "talk", "erosion", "erosion_danger", "no_house", "heal"
]
...
func _on_periodic_timer_timeout() -> void:
	...
	if phase == Researcher3D.Phase.WORK:
		pool = ["idle", "confuse", "work"]
	else:
		pool = ["idle", "confuse", "talk", "good"]
	if no_house:
		pool.append("no_house")
	if eroded:
		pool.append("erosion")
		pool.append("erosion_danger")
	if healing:
		pool.append("heal")
	if pool.is_empty():
		pool = ["idle"]
