...
@export var core_consumption_title_path: NodePath = NodePath("DetailsVboxContainer/ContentMargin/ContentVbox/CoreConsumptionWrap/CoreConsumption/CoreConsumptionTitle/Label")
@export var core_consumption_value_path: NodePath = NodePath("DetailsVboxContainer/ContentMargin/ContentVbox/CoreConsumptionWrap/CoreConsumption/CoreConsumptionTitle/Value")
...
## 显示面板并绑定数据
## 计算因子特殊处理：消耗组为「核心消耗」、仅 title 层无子条目；显示预期总消耗、固有消耗、核心消耗、产出
func show_panel(data: Dictionary) -> void:
	...
	## 核心消耗（计算因子特殊处理：仅标题层，无子条目）
	var core_entries: Array = factor_data.get("archives_consumption", [])
	_update_core_consumption(core_entries)
...
func _update_core_consumption(entries: Array) -> void:
	## 计算核心消耗总计（计算因子：来自核心房间的消耗）
	var total := 0
	for entry in entries:
		total += entry.get("amount", 0)
	
	## 更新核心消耗标题
	var label := get_node_or_null(core_consumption_title_path) as Label
	if label:
		label.text = tr("FACTOR_CORE_CONSUMPTION")
	
	## 更新核心消耗数值
	var value_label := get_node_or_null(core_consumption_value_path) as Label
	if value_label:
		value_label.text = str(total) + "/天"
	
	## 计算因子特殊处理：核心消耗仅 title 层，无子条目
	## 不需要动态生成子条目，所有消耗都汇总显示在标题层
...
func _update_title() -> void:
	...
	if not title_label:
		return
	var title_text := tr("LABEL_COMPUTATION")
	if title_text == "LABEL_COMPUTATION" or title_text.is_empty():
		title_text = "计算因子"
...
func _update_storage_title() -> void:
	...
	if not storage_title_label:
		return
	var title_text := tr("DETAIL_STORAGE_TITLE_COMPUTATION")
	if title_text == "DETAIL_STORAGE_TITLE_COMPUTATION" or title_text.is_empty():
		title_text = "计算因子储存服务器"
