...
- **侵蚀序列**：基于 `ErosionCore.get_erosion_at_game_hour()`；当前实现为**全程固定显性 -2**（不再按周期随机五种等级）
...
- **handle**：仅侵蚀变化点生成；恶化→红标、好转→绿标（神秘侵蚀恒为 -2 时无新变化点，新开局通常无 handle；旧档读入的 handle 仍按原规则右移直至消失）
