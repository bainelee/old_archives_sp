...
- **因子区域**（认知、计算、意志、权限）：悬停显示 FactorHoverPanel，在鼠标左侧展示：
  - 因子库存（当前/上限，如 59,000 / 60,000）
  - 总消耗与可供使用天数（如 总消耗：144/天 可供使用 833天）
  - 消耗细则（按区域-房间名，如 核心区-档案馆核心 72/天）
  - 产出细则（按区域-房间名，如 研究区-终端操作台 360/天）
