...
**前置条件**：房间必须处于**已清理**状态方可建设（见 [10 - 房间清理系统](10-room-cleanup-system.md)）。
...
| 区域类型 | 可建设于的房间类型 | 备注 |
|----------|-------------------|------|
| 研究区 | 图书室、机房、资料库、教学室 | 消耗房间存量产出因子，数值见 [08](08-game-values.md) 第 6 节 |
...
- 建设时长 = 房间单位数 × 每单位耗时（见 [08 - 游戏数值系统](08-game-values.md)）
- 建设期间房间不可进行其他建设或改造操作
...
**后续逻辑**：已建设房间的详细运作、产出节奏、存量耗尽与停工等，见 [12 - 已建设房间系统](12-built-room-system.md)。
...
建设消耗、耗时等数值详见 [08 - 游戏数值系统](08-game-values.md) 第 5 节「建设区域」，此处仅作引用。
...
**待定**：与 [07 - 存档系统](07-save-system.md) 和 `room_info.json` 同步规则结合后确定。
...
建设选择状态与 [10 - 房间清理系统](10-room-cleanup-system.md) 的清理选择状态采用**相同交互模式**：
...
鼠标移动到区域按钮上时，显示 tooltip 或悬浮窗，内容来源 [08 - 游戏数值系统](08-game-values.md) 第 5 节：
...
| 房间名称 | |
| 房间类型 | |
| 建设后每小时产出 | 研究区/造物区等产出数据，见 [08](08-game-values.md) 第 6、7 节 |
...
本节根据 [10 - 房间清理系统](10-room-cleanup-system.md) 的设计、已实现代码及其中遇到的坑，给出区域建设的技术实现步骤与注意事项。实现时**务必参考清理系统的实现模式**，复用相同架构。
...
**需排除/放行的 UI**（参考 [10 - 房间清理系统](10-room-cleanup-system.md) 第 8 节）：
...
- [08 - 游戏数值系统](08-game-values.md)（建设消耗、耗时、研究区/造物区产出）
- [10 - 房间清理系统](10-room-cleanup-system.md)（清理前置、研究员占用模式、经验小结）
- [04 - 时间流逝系统](04-time-system.md)
- [07 - 存档系统](07-save-system.md)
- [关键词对照](../settings/00-project-keywords.md)（区域类型、房间类型）
- [12 - 已建设房间系统](12-built-room-system.md)：已建设房间的持续运作、产出节奏、存量耗尽、停工等详细逻辑
