...
| 维度 | 研究区 | 造物区 |
|------|--------|--------|
| **房间资源存量** | 有，建设前即存在于 `room.resources`（见 1.5 房间存量表） | 无基础资源储存 |
...
### 1.3 产出速率（1 单位，01-game-values §6.1）

数据来源：`datas/game_values.json` 的 `research_output`。
...
### 1.5 房间存量配置（room_info.json）

研究区房间在 `room_info.json` 的 `room_resources` 中配置存量，建设研究区后按小时消耗并产出因子。
...
- **消耗**：玩家意志因子（每单位 5/h）
- **产出**：实验室产出权限、推理室产出信息（每单位 5/h）
- **节奏**：每游戏小时检查意志是否充足；充足则扣除意志、增加产出；不足则**停工**，不产出

### 2.3 产出速率（1 单位，01-game-values §7.1）

数据来源：`datas/game_values.json` 的 `creation_output`。
...
- **住房**：每 2 单位居住区提供 4 住房（公式：`housing = room_units × 2`）；当前宿舍均为 base 型（2 单位）→ 每房间 4 住房
- **住房需求**：1 名研究员需 1 住房
