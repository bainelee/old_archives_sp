...
| 游戏主场景 | 房间 + 时间 + 资源 | `user://saves/slot_N.json` | 新游戏从 room_info.json 生成房间，不再用 maps |
...
│  - 新游戏：从 room_info.json 加载带 grid 的房间，写入 saves               │
└─────────────────────────────────────────────────────────────────────┘
```
...
- **新游戏初始化**：从 `room_info.json` 加载带 grid 的房间（含 `layout_cells`），由 `layout_cells` 派生邻接并应用开篇，叠加 game_base 默认值后写入选定存档槽位；不再使用 maps/slot_0。
