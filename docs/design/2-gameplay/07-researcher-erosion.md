...
侵蚀本身是全局的（`ErosionCore.current_erosion` = 神秘侵蚀 + 文明的庇佑，与 TopBar 一致）；研究员所受庇护取决于**工作分配**与**住房分配**：

| 工作 | 住房 | 庇护等级来源 | 特殊 |
|------|------|-------------|------|
| 无 | 无 | 全额侵蚀（无庇护） | — |
| 无 | 有 | 宿舍庇护等级 | 有住房但无工作，按宿舍计 |
| 有 | 有 | 未侵蚀→工作房；已侵蚀→宿舍 | 治愈用宿舍庇护等级 |
| 有 | 无 | 工作房庇护等级 | **无住房** 标签 → 侵蚀概率×2 |

- **无住房**：有工作但无住房时附加此标签，侵蚀判定概率加倍

### 1.2 每日侵蚀判定

当研究员**有效庇护等级**为**薄弱**、**暴露**或**绝境**时，该研究员每天会进行一次侵蚀概率判定。
...
- **过程**：随机 1～100，若随机数 ≤ 当前侵蚀概率（**无住房**时×2），则为该研究员附加一个 **侵蚀风险** 标记
...
- 生活区宿舍的庇护等级由核心产出的庇护能量分配至该房间决定（见 [01 §2](../0-values/01-game-values.md#2-档案馆核心庇护能量与计算因子消耗)）
- **无住房** 的研究员若被侵蚀，**无法**被治愈（治愈判定时跳过），只能等待死亡或其他方式
...
|--------|------|
| id | 研究员唯一编号，用于排序消耗认知 |
| work_room_id | 当前工作所在房间；空表示无工作（清理/建设中或空闲） |
| housing_room_id | 分配的住房所在宿舍；空表示无住房 |
| has_no_housing | 有工作但无住房时为 true，侵蚀概率×2 |
| 侵蚀风险标记数 | 每 7 天结算时清零（若未侵蚀）或随侵蚀触发清零 |
| 认知危机标记数 | 0～3，达到 3 则认知失能 |
| 被侵蚀天数 | 用于死亡概率计算 |
| prev_room_id | 治愈后返回的工作房间 |
