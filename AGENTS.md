...
- **探索（P1）**：`datas/exploration_config.json`、`datas/exploration_investigations.json`；`scripts/game/exploration/`（`exploration_service.gd`、`exploration_tick.gd`、`exploration_rules.gd`、`exploration_state_codec.gd`）；`exploration_map_overlay.*`、`exploration_region_info_panel.*`、`exploration_investigation_event_panel.*`；存档根键 `exploration`（v3 含 `completed_investigation_site_ids`）；GameplayFlow：`flows/suites/regression/gameplay/exploration_two_regions_save_verify.json`；说明见 `docs/design/2-gameplay/10-exploration-region-map.md`、`docs/testing/09-exploration-gameplayflow-validation.md`
